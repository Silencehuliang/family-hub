...
export function BillEditPageDesktop() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit && id) {
        await updateBill.mutateAsync({ id, amount: amountNum, categoryL1: selectedL1, categoryL2: selectedL2, payerId, billDate, note: note || undefined, tags: selectedTags });
        message.success('已更新');
      } else {
        await createBill.mutateAsync({ amount: amountNum, categoryL1: selectedL1, categoryL2: selectedL2, payerId, billDate, note: note || undefined, tags: selectedTags });
        message.success('已记录');
      }
      navigate('/bill');
  ...
  return (
    <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
      ...
      <div style={{ textAlign: 'center', margin: '24px 0' }}>
        ...
        <Input prefix="¥" value={amount} onChange={(e) => setAmount(e.target.value.replace(/[^\d.]/g, ''))} placeholder="0.00" style={{ fontSize: 28, textAlign: 'center' }} size="large" autoFocus={!isEdit} />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, margin: '8px 0' }}>
        {l1Categories.filter((c) => !c.hidden).map((cat) => (
          <div key={cat.id} onClick={() => handleSelectL1(cat)}
            style={{ textAlign: 'center', padding: '8px 4px', borderRadius: 8, cursor: 'pointer', background: selectedL1 === cat.id ? `${cat.color}20` : '#f5f5f5', border: selectedL1 === cat.id ? `2px solid ${cat.color}` : '2px solid transparent' }}
          >
        ...
        <div style={{ margin: '12px 0', padding: 12, background: '#f9f9f9', borderRadius: 8 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
            {l2Categories.map((cat) => (
              <Tag key={cat.id} color={selectedL2 === cat.id ? selectedL1Cat?.color : undefined} style={{ cursor: 'pointer', padding: '4px 12px' }} onClick={() => handleSelectL2(cat)}>{cat.icon} {cat.name}</Tag>
            ))}
          </div>
        </div>
      )}
      ...
      <div style={{ margin: '12px 0' }}>
        ...
        <DatePicker value={dayjs(billDate)} onChange={(d) => d && setBillDate(d.format('YYYY-MM-DD'))} style={{ width: '100%' }} size="large" />
      </div>
        ...
        <div style={{ margin: '12px 0' }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
            {tags.filter((t) => !t.archived).map((tag) => (
              <Tag.CheckableTag key={tag.id} checked={selectedTags.includes(tag.id)}
                onChange={(checked) => { setSelectedTags((prev) => checked ? [...prev, tag.id] : prev.filter((id) => id !== tag.id)); }}
      ...
      <div style={{ margin: '12px 0' }}>
        ...
        <Input.TextArea value={note} onChange={(e) => setNote(e.target.value)} placeholder="可选备注" maxLength={200} rows={2} />
      </div>

      <Space style={{ marginTop: 16, width: '100%' }} direction="vertical">
        <Button type="primary" size="large" block loading={createBill.isPending || updateBill.isPending} onClick={handleSubmit}>{isEdit ? '保存修改' : '记一笔'}</Button>
