...
function getEventColor(type: string): string {
  return EVENT_COLORS[type] ?? '#DDA0DD';
}

function getDateStr(d: dayjs.Dayjs): string {
...
function eventsForDay(events: EventItem[], day: dayjs.Dayjs): EventItem[] {
  ...
  return events.filter(e => {
    if (e.allDay) {
      const eDay = dayjs.unix(e.startAt).format('YYYY-MM-DD');
      return eDay === getDateStr(day);
    }
...
export function CalendarPage() {
  ...
  const grid = useMemo(() => {
    ...
    for (let i = 0; i < startDow; i++) {
      cells.push({ date: start.subtract(startDow - i, 'day'), isCurrentMonth: false });
    }

    const daysInMonth = current.daysInMonth();
    for (let d = 1; d <= daysInMonth; d++) {
      cells.push({ date: start.date(d), isCurrentMonth: true });
    }
  ...
  return (
    <div style={{ padding: 16, maxWidth: 768, margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/calendar/new')}>
          新建
        </Button>
      </div>
      ...
      {isLoading ? (
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 1, background: '#f0f0f0', borderRadius: 8, overflow: 'hidden' }}>
            {WEEK_DAYS.map(d => (
              <div key={d} style={{ background: '#fafafa', textAlign: 'center', padding: '8px 4px', fontWeight: 600, fontSize: 12, color: '#888' }}>
                {d}
              </div>
            ))}
            {grid.map((cell, i) => {
              ...
              const isSelected = selectedDay && getDateStr(selectedDay) === dateStr;
              ...
              return (
                <div
                  ...
                  style={{
                    background: cell.isCurrentMonth ? '#fff' : '#f9f9f9',
                    minHeight: 64,
                    padding: 4,
                    cursor: 'pointer',
                    borderBottom: isSelected ? '2px solid #1677ff' : undefined,
                ...
                >
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                    {dayEvents.slice(0, 2).map(e => (
                      <div
                        ...
                        style={{
                          ...
                          padding: '1px 3px',
                          borderRadius: 3,
                          background: getEventColor(e.type) + '30',
                    ...
                    {dayEvents.length > 2 && (
                      <Text style={{ fontSize: 10, textAlign: 'center', color: '#999' }}>+{dayEvents.length - 2}</Text>
                    )}
                  </div>
                </div>
              );
