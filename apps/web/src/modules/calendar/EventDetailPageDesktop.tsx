...
export function EventDetailPageDesktop() {
  ...
  if (!event) return <div style={{ padding: 16, textAlign: 'center' }}><Text type="secondary">日程不存在</Text><div style={{ marginTop: 16 }}><Button onClick={() => navigate('/calendar')}>返回日历</Button></div></div>;
  ...
  return (
    <div style={{ padding: 16, maxWidth: 480, margin: '0 auto' }}>
      <Button type="text" icon={<ArrowLeftOutlined />} onClick={() => navigate('/calendar')} style={{ marginBottom: 16 }}>返回</Button>
      ...
      <Space direction="vertical" size="small" style={{ width: '100%' }}>
        <div><Text type="secondary" style={{ fontSize: 12 }}>开始时间</Text><div><Text>{startStr}</Text></div></div>
        {endStr && <div><Text type="secondary" style={{ fontSize: 12 }}>结束时间</Text><div><Text>{endStr}</Text></div></div>}
        {event.location && <div><Text type="secondary" style={{ fontSize: 12 }}>地点</Text><div><Text>{event.location}</Text></div></div>}
        {event.note && <div><Text type="secondary" style={{ fontSize: 12 }}>备注</Text><div><Text>{event.note}</Text></div></div>}
        {event.participants && event.participants.length > 0 && (
          <div><Text type="secondary" style={{ fontSize: 12 }}>参与成员</Text><div><Text>{event.participants.map(p => p.nickname).join(', ')}</Text></div></div>
        )}
      </Space>
      ...
      <Space>
        <Button type="primary" icon={<EditOutlined />} onClick={() => navigate(`/calendar/${event.id}/edit`)}>编辑</Button>
        <Popconfirm title="确定删除此日程？" onConfirm={handleDelete} okText="删除" cancelText="取消">
          <Button danger icon={<DeleteOutlined />} loading={deleteEvent.isPending}>删除</Button>
        </Popconfirm>
      </Space>
    </div>
  );
