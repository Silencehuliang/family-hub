...
export class CategoryService {
  ...
  async getTree(familyId: string): Promise<{ l1: BillCategory[]; l2: BillCategory[] }> {
    ...
    return { l1: camelCaseAll<BillCategory>(l1Raw), l2: camelCaseAll<BillCategory>(l2Raw) };
  }
  ...
  async create(familyId: string, input: { parentId: string; name: string; icon?: string; color?: string }): Promise<BillCategory> {
    ...
    return findOne<Record<string, unknown>>(this.db, 'SELECT * FROM bill_category WHERE id = ?', id)
      .then((r) => r ? camelCase<BillCategory>(r) : null) as Promise<BillCategory>;
  }
